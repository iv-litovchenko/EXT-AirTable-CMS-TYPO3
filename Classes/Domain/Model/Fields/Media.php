			...
			if($v['deleted'] == 1) {
				...
				$content .='
					<tr>
						...
						<td nowrap style="vertical-align: top; border-right: 0; background: #eee;"><nobr><span style="">'.$icon.' R'.$v['uid'].', F'.$v['file']['uid'].' <!--'.$j.'/'.count($row[$fieldInRow]).'--></span></nobr></td>
					</tr>
				';
			} else {
	...
	*/
	public static function FalFilePath($rowArray = [])
	{
		if(isset($rowArray['storage']['configuration'])){
			$storageFile = $rowArray['storage']['configuration']['basePath'];
	...
	public function refProvider($obj, $class, $field){
		...
		if($annotationField == 'Media_1'){
			...
			->with(['file'=>function($query){ // ---> uid_local AS `file`
