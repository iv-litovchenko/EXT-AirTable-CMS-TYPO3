...
```
1) Посмотри почему не работает расширение модели связи префикс?
2) Названия колонок - props, attrs, tbl_, field....??
      Атрибуты относятся к дополнительной информации об объекте. 
      Свойства описывают характеристики объекта. 
3) Как менять атрибуты TCA EAV?
4) Выборка + keyBy???
...
5) Когда делаем две связи на одну и туже таблицу нужно создавать разные FK-ключи... на примере tx_marker_rows
6) TYPO3 8
```
