1. + Сущности допилить (их всего 4)
2. -> Категории Data + стандартные поля придумать как их включать... (осталось только на форме редактирования TCEFORM)
3. -> Вывод полей для pages (+),tt_content (придумать как сделать из-за того что три типа плагин, сетка, элемент)
4. -> Добавил LIKE // Проверить работу связи EAV когда убрал where (будул ли свойства правильно выводится если у сущностей одинаковые ID)
5. Выборка Data + атрибуты + фильтрация
6. Поля (атрибуты) в контроллерах...
7. // Todo - общие поля для всех типов сущности На примере страниц - будут показываться во всех шаблонах
8. То поле Flexform
