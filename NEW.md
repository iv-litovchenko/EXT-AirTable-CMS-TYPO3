```
2) Трайты (продумать их реализцию для моделей Crud CrudOvveride
3) Передумать название "CrudOvveride")
4) EX (сверить)
5) Удалить аннотации...  * @AirTable\Field:
6) Посмотреть SQL Анализер (запустить и посмотреть будут ли изменения в БД?)
